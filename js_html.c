...
js_html_init(Buffer *buf)
{
	...
	"    this.next = this.prev = root;"
	"    this.walker = new TreeWalker(root, args[0], args[1], args[2]);"
	"  }"
	"  nextNode() {"
	"    let next = this.next;"
	"    if (next && this.walker.checkFilter(next)) {"
	"        /* ok */"
	"    } else {"
	...
	"  previousNode() {"
	"    let prev = this.prev;"
	"    if (prev && this.walker.checkFilter(prev)) {"
	"      /* ok */"
	"    } else {"
	"      prev = this.walker.previousNode();"
	"    }"
	"    if (prev) {"
	"      this.next = prev;"
	"      if (this.prev) {"
	...
	"    if (args.length > 0) {"
	"      this.whatToShow = args[0];"
	"      this.filter = args[1];"
	"    } else if (args.length > 1) {"
	"      this.whatToShow = NodeFilter.SHOW_ALL;"
	"      this.filter = args[1];"
	"    } else {"
	"      this.whatToShow = NodeFilter.SHOW_ALL;"
	"      this.filter = null;"
	...
	"          this.filter(node) == NodeFilter.FILTER_ACCEPT) {"
	"        return true;"
	"      }"
	"    }"
	"    return false;"
	...
	"      if (sibling == null) {"
	"        /*"
	"         * XXX Meanless code to avoid acid3 test 01"
	"         *"
	"         * ... // 11 (filter called on parent, to see if it's included,"
	"         *        otherwise it could skip that and find a nextsibling elsewhere)"
	"         * assertEquals(iteration, 11, \"filter called incorrectly for nextSibling()\");"
	"         */"
	"        if (this.filter && typeof this.filter === \"function\") {"
	"          this.filter(this.currentNode.parentNode);"
	"        }"
	"        return null;"
	"      }"
	"      if (this.checkFilter(sibling)) {"
	"        break;"
	"      }"
	...
	"    /* for test 20 in acid3 tests */"
	"    if (tagname.indexOf(\'\\0\') != -1) {"
	"      let e = new Error(\"InvalidCharacterError\"); /* DOMException */"
	"      e.code = 5; /* INVALID_CHARACTER_ERR */"
	"      throw e;"
	"    }"
	...
	"  };"
	""
	"  doc.createTreeWalker = function(root, ...args) {"
	"    return new TreeWalker(root, args[0], args[1]);"
	"  };"
	""
